...
## Overview
A professional-grade mathematical simulation platform with retro-terminal CRT/Matrix aesthetic. Features high-quality image export and 17 complex mathematical visualizations. Designed for data scientists, computational artists, and R&D teams.

## Recent Changes (December 16, 2025)
- Migrated all audio files to Cloudflare R2 CDN
- Deleted local music folder and attached_assets (screenshots/duplicates)
- Site now serves as static files from public/ folder
- Fixed R2 URLs to use public endpoint (pub-*.r2.dev) for proper CORS access

## Tech Stack
- **Frontend**: Vanilla JS with Canvas API + WebGL2
- **Styling**: CSS with CRT/Matrix visual effects
- **Audio**: Cloudflare R2 CDN (`pub-14b82f67a06cec7782ea14ad4ac98073.r2.dev/bucketz/`)
- **Deployment**: Static file serving via `serve` package
